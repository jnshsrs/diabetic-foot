...
prior_information <- list("PAD" = fit_pad, 
     "Ulcer size" = fit_size,
     "Ulcer depth" = fit_depth) %>%
  map(broom::tidy) %>%
  bind_rows(.id = "model") %>%
...
# pool prior information (across all paramter)

pooled_avg <- prior_information$beta %>% mean
pooled_sterr <- prior_information$std.error %>% mean 

# Calculate pooled Standard Deviation
pooled_sd <- pooled_sterr * qnorm(p = .975) ## 95% confidence interval of the pooled OR
# Calculate conservative pooled 
pooled_beta <- pooled_avg - (2 * pooled_sterr)
